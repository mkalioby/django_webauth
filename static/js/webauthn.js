  ...
  if (PublicKeyCredential &&
    ...
    PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable().then(response => {
      if (response === true) {
        ...
        jQuery("#div").html('<a href="javascript:void(0)" onclick="startCreate()">Create Auth</a>')

      } else {
        jQuery("#div").html(`User verifying platform authenticator is NOT available.`);
      }
    }).catch(err => {
      jQuery("#div").html(`UVPAA failed: [`+err.toString()+`]`);
    });
  } else {
    jQuery("#div").html(`User verifying platform authenticator is not available on this browser.`);
  }
}
